...
@frappe.whitelist()
def mark_att_one_employee_ot():
    
    from_date= "2025-02-16"
    
    to_date= "2025-02-16"
    
    checkins = frappe.db.sql("""select * from `tabEmployee Checkin` where date(time) between '%s' and '%s' AND employee='111' order by time ASC """%(from_date,to_date),as_dict=True)
    ...
    ot_calculation_holiday(from_date,to_date)
    update_ot_request(from_date,to_date)
...
def ot_calculation(from_date,to_date):
    attendance = frappe.db.sql("""select * from `tabAttendance` where attendance_date between '%s' and '%s' and docstatus =0"""%(from_date,to_date),as_dict=True)
...
def ot_calculation_holiday(from_date,to_date):
    attendance = frappe.db.sql("""select * from `tabAttendance` where attendance_date between '%s' and '%s' and docstatus =0 AND custom_employee_category = '' """%(from_date,to_date),as_dict=True)
    for att in attendance:
        ot_hours = '00:00:00'
        if att.shift and att.out_time and att.in_time :
            hh = check_holiday(att.attendance_date,att.employee)    
...
def ot_calculation_holiday_employee(from_date,to_date,employee):
    attendance = frappe.db.sql("""select * from `tabAttendance` where attendance_date between '%s' and '%s' and employee = '%s' and custom_employee_category = " " """%(from_date,to_date,employee),as_dict=True)
    for att in attendance:
        ot_hours = '00:00:00'
        if att.shift and att.out_time and att.in_time :
            hh = check_holiday(att.attendance_date,att.employee)
...
def get_actual_shift(get_shift_time,employee,att_date):
    ...
    if actual_shift=='Single':
        ...
        shift_assignment = frappe.db.get_value("Shift Assignment",{"employee":employee,"start_date":att_date},["shift_type"])
        shift_start_time = frappe.db.get_value('Shift Type',{'name':shift_assignment},['custom_checkin_start_time','custom_checkin_end_time'])
        if shift_assignment and shift_start_time[1]:
            # shift_start_time = datetime.min + shift_start_time[0] 
            if (datetime.min + shift_start_time[0]).time() < get_shift_time < (datetime.min + shift_start_time[0]).time():
                shift = shift_assignment
            else:
                        ...
                        if wh > 0 :
                            ...
                            if wh < ab_threshold:
                                if att.leave_application:
                                    if frappe.db.get_value("Leave Application",{"name":att.leave_application},["half_day"]) == 1:
                                        if frappe.db.get_value("Leave Application",{"name":att.leave_application},["half_day_date"]) == from_date or frappe.db.get_value("Leave Application",{"name":att.leave_application},["half_day_date"]) == to_date:
...
def update_att_with_employee(from_date,to_date,employee):
    ...
    ot_calculation_holiday_employee(from_date,to_date,employee)
    update_ot_request(from_date,to_date)
    return "ok"
...
def mark_whs(from_date,to_date,employee):
    attendance = frappe.db.get_all('Attendance',{'attendance_date':('between',(from_date,to_date)),'docstatus':('!=','2'),'employee':employee,"status":("!=","Holiday")},['*'])
    for att in attendance:
        att.custom_ot_hours = "00:00:00"
        att.custom_extra_hours_total ="00:00:00"
...
def mark_wh_without_employee(from_date,to_date):
    attendance = frappe.db.get_all('Attendance',{'attendance_date':('between',(from_date,to_date)),"status":("!=","Holiday"),'docstatus':('!=','2')},['*'])
    for att in attendance:
        employement_type=frappe.db.get_value("Employee",{"name":att.employee},["employment_type"])
        if not employement_type == "Agency":
            if att.shift and att.in_time and att.out_time :
...
def mark_cc(from_date,to_date):
    attendance = frappe.db.get_all('Attendance', filters={
        'attendance_date': ['between', (from_date, to_date)],"status":("!=","Holiday"),
        'docstatus': ['!=', '2']
            ...
            if default_shift:
                if default_shift == att.shift:
                    ...
                    if wh > 0 :
                        ...
                        if wh < ab_threshold:
                            if att.leave_application:
                                if frappe.db.get_value("Leave Application",{"name":att.leave_application},["half_day_date"]) == att.attendance_date:
                                    frappe.db.set_value('Attendance',att.name,'status','Half Day')
                                else:
                                    frappe.db.set_value('Attendance',att.name,'status','On Leave')
...
def grace_late_time(to_date):
    ...
    attendance_records = frappe.get_all(
        ...
        fields=["name","working_hours","custom_on_duty_application", "employee", "custom_attendance_permission","attendance_date", "custom_late_entry_time", "early_exit", "status"]
    )
        ...
        for record in records:
            ...
            if processed_days < 3 and custom_late_entry_time <= late_entry_threshold:
                if record["early_exit"] == 0 :
                    if record["status"] != "Present" and record["working_hours"] > '8.2':
                        frappe.db.set_value("Attendance", record["name"], "status", "Present")
                    processed_days += 1
                    
            else:
                if not record['custom_attendance_permission'] and not record['custom_on_duty_application']:
                    if record["status"] == "Present":
                        frappe.db.set_value("Attendance", record["name"], "status", "Half Day")
