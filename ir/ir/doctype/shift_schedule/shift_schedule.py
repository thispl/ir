...
class ShiftSchedule(Document):
    ...
    def enqueue_submit_schedule(self,ss,workflow_state,fd,td,file):
        if workflow_state == "Approved":
            ...
            sas = frappe.get_all("Shift Assignment",{'custom_shift_schedule':ss,'docstatus':'0'})
            for sa in sas:
    ...
    def validate(self):
        ...
        if self.is_new():
            for date in dates:
                if frappe.db.exists("Shift Schedule", {"from_date": ["<=", date],"to_date": [">=", date],"department":self.department,"docstatus":("!=",2)}):
        ...
        for pp in pps:
            if pp[0] != 'ID':
                ...
                employee_name = pp[0]
                department = pp[2]
                employee_category = pp[3]

                emp_department = frappe.db.get_value("Employee", {"name": employee_name}, "department")
                emp_employee_category = frappe.db.get_value("Employee", {"name": employee_name}, "custom_employee_category")

                error_messages = []
                if employee_category != emp_employee_category:
                    error_messages.append(f"{employee_name}-{pp[1]} is not assigned to the {employee_category} Employee Category")
                if department != emp_department:
                    error_messages.append(f"{employee_name}-{pp[1]} is not assigned to the {department} Department")
    ...
    def validate_employees(self):
        ...
        for pp in pps:
            if pp[0] != 'ID':
                emp_list.append(pp[0])
                if pp[2] != frappe.db.get_value("Employee",{"name":pp[0]},["department"]):
                    frappe.throw(f"{pp[0]}-{pp[1]} is not in this Department {pp[2]}")
    ...
    def show_summary(self):
        ...
        total = wc1 + wc2 + wc3 + wcpp1 + bc1 + bc2 + bc3 + bcpp1 + ft1 + ft2 + ft3 + ftpp1
        data += """
            <table style="border-collapse: collapse; width: 100%;">
                ...
                <tr>
                    <th style="border: 1px solid black">Grey Collar</th>
                ...
                <tr>
                    <th style="border: 1px solid black">White Collar</th>
                    <td style="border: 1px solid black">{}</td>
                    <td style="border: 1px solid black">{}</td>
                    <td style="border: 1px solid black">{}</td>
                    <td style="border: 1px solid black">{}</td>
                ...
                <tr>
                    <th style="border: 1px solid black">Blue Collar</th>
                    <td style="border: 1px solid black">{}</td>
                    <td style="border: 1px solid black">{}</td>
                    <td style="border: 1px solid black">{}</td>
                    <td style="border: 1px solid black">{}</td>
        ...
        """.format(
            ...
            bc1, bc2, bc3, bcpp1, bc1 + bc2 + bc3 + bcpp1,
            ft1, ft2, ft3, ftpp1, ft1 + ft2 + ft3 + ftpp1,
            wc1 + bc1 + ft1, wc2 + bc2 + ft2, wc3 + bc3 + ft3, wcpp1 + bcpp1 + ftpp1, total
        )
            ...
            for date in dates:
                if not frappe.db.exists("Shift Assignment", {'employee': pp[0], 'start_date': date, 'end_date': date, 'docstatus':("!=",2)}):
                    if not frappe.db.exists("Shift Request", {'employee': pp[0], 'from_date': date, 'to_date': date, 'docstatus':("!=",2)}):
                        ...
                        doc.start_date = date
                        doc.end_date = date
                        doc.custom_shift_schedule = name
...
def get_data(args):
    ...
    for employee in employees:
        row = [
            employee.name, employee.employee_name, employee.department,employee.custom_employee_category,employee.default_shift
        ]
        data.append(row)
    return data
...
def shift_wise_count(doc):
    ...
    data += '<tr><td style="background-color:#f0b27a">Total</td><td>%s</td><td style="background-color:#f0b27a">Total</td><td>%s</td><td style="background-color:#f0b27a">Total</td><td>%s</td><td style="background-color:#f0b27a">Total</td><td>%s</td><td style="background-color:#f0b27a">Total</td><td>%s</td><td style="background-color:#f0b27a">Total</td><td>%s</td></tr>'%(sf1[0].count,sf2[0].count,sf3[0].count,spp1[0].count,spp2[0].count,sf1[0].count+sf2[0].count+sf3[0].count+spp1[0].count+spp2[0].count)
    data = data + '</table>' 
    return data
...
def shift_cancel(doc,method):
    sas = frappe.get_all("Shift Assignment", filters={'custom_shift_schedule': doc.name, 'docstatus': 1}, fields=['name'])
    for sa in sas:
        doc = frappe.get_doc('Shift Assignment', sa.name)
        doc.cancel()
    frappe.msgprint('Shift Schedule Rejected Successfully')
