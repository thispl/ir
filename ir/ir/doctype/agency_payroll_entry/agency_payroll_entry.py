...
# class AgencyPayrollEntry(Document):
# 	pass
...
class AgencyPayrollEntry(Document):
	...
	def cancel(self):
		if len(self.get_linked_salary_slips()) > 50:
			msg = _("Agency Payroll Entry cancellation is queued. It may take a few minutes")
			msg += "<br>"
			msg += _(
				"In case of any error during this background process, the system will add a comment about the error on this Agency Payroll Entry and revert to the Submitted status"
			)
			frappe.msgprint(
				msg,
				indicator="blue",
	...
	def make_filters(self):
		filters = frappe._dict(
			...
			branch=self.branch,
			custom_agency_name=self.custom_agency_name,
			...
			employment_type = self.custom_employment_type,
	...
	def fill_employee_details(self):
		...
		if not employees:
			...
			if self.custom_agency_name:
				error_msg += "<br>" + _("Branch: {0}").format(frappe.bold(self.custom_agency_name))
			if self.department:
				error_msg += "<br>" + _("Department: {0}").format(frappe.bold(self.department))
			if self.designation:
				error_msg += "<br>" + _("Designation: {0}").format(frappe.bold(self.designation))
			if self.custom_agency_name:
				error_msg += "<br>" + _("custom_agency_name: {0}").format(frappe.bold(self.custom_agency_name))
			if self.start_date:
...
def set_fields_to_select(query, fields: list[str] = None):
	default_fields = ["employee", "employee_name", "department", "designation","custom_agency_name","employment_type"]
...
def set_filter_conditions(query, filters, qb_object):
	...
	for fltr_key in ["branch", "department", "designation", "grade","custom_agency_name","employment_type"]:
...
def get_payroll_entry_bank_entries(payroll_entry_name):
	...
	journal_entries = (
		...
		.where(
			...
			& (jea.reference_type == "Agency Payroll Entry")
		)
	).run(as_dict=True)
...
def log_payroll_failure(process, payroll_entry, error):
	error_log = frappe.log_error(
		title=_("Salary Slip {0} failed for Agency Payroll Entry {1}").format(process, payroll_entry.name)
	)
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
	payroll_entry = frappe.get_cached_doc("Agency Payroll Entry", args.payroll_entry)
...
def get_payroll_entries_for_jv(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql(
		"""
		select name from `tabAgency Payroll Entry`
		where `{key}` LIKE %(txt)s
		and name not in
			(select reference_name from `tabJournal Entry Account`
				where reference_type="Agency Payroll Entry")
