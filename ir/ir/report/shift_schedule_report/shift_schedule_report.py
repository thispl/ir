...
def add_data(employee_map, att_map, filters, holiday_map, conditions, default_holiday_list, leave_list=None):
	...
	for emp in employee_map:
		...
		if filters.group_by:
			row += [" "]
		row += [emp, emp_det.employee_name, emp_det.department]
...
def get_columns(filters):
	...
	columns += [
		_("Employee") + ":Link/Employee:120", _("Employee Name") + ":Data/:120",_("Department") + ":Link/Department:120"
	]
	days = []
	for day in range(filters["total_days_in_month"]):
...
def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(start_date) as day_of_month,department,
		shift_type from `tabShift Assignment` where docstatus = 1 %s order by employee, start_date""" %
		conditions, filters, as_dict=1)

	if not attendance_list:
		msgprint(_("No Shift Assignment record found"), alert=True, indicator="orange")

	att_map = {}
	for d in attendance_list:
		att_map.setdefault(d.employee, frappe._dict()).setdefault(d.day_of_month, "")
		att_map[d.employee][d.day_of_month] = d.shift_type
...
def get_conditions(filters):
	...
	conditions = " and month(start_date) = %(month)s and year(start_date) = %(year)s"
...
def get_employee_details(group_by, company):
	emp_map = {}
	query = """select name, employee_name, designation, department, branch, company,grade,
...
def get_attendance_years():
	year_list = frappe.db.sql_list("""select distinct YEAR(start_date) from tabAttendance ORDER BY YEAR(start_date) DESC""")
	if not year_list:
