...
def execute(filters=None):
	...
	for ss in salary_slips:
		row = {
			...
			"paid_holiday":ss.custom_agency_paid_holiday,
...
def get_columns(earning_types, ded_types):
	columns = [
		...
		{
			"label": _("Paid Holiday"),
			"fieldname": "paid_holiday",
			"fieldtype": "Float",
			"width": 120,
		},
		{
	...
	ordered_earnings = ["Agency Basic","Stipend", "Agency DA", "Agency Special Allowance","OT","Attendance Bonus"]
	ordered_deductions = ["Agency PF", "Agency ESI","Canteen"]
...
def get_salary_slips(filters, company_currency):
    ...
    .where(
        (employee.custom_agency_employee == 1) &
    ...
    .select(
        ...
        salary_slip.custom_agency_paid_holiday,
        salary_slip.branch,
        employee.custom_agency_employee
    )
)
